...
\nomenclature{$a_{ij}$}{Stoichiometric coefficients, e.g. number of atoms of element $i$ in species $j$. Weighted by element $i$'s molecular weight}
\nomenclature{$\hat{R}$}{Universal gas constant $\approx$ 8314 kJ/kg-K}
\nomenclature{$b_i$}{Sum of contributions from each species towards constraint for element $i$, i.e. $\sum_{j=1}^\text{NS} a_{i,j}\mathcal{N}_j$}
...
\nomenclature{$\mathcal{M}_j$}{Molecular weight of species $j$ in kg / kg-mole}
...
\nomenclature{$b_i^\circ$}{Specified mass fraction of element $i$}
...
\nomenclature{$u'$}{Sum of contributions from each species towards internal energy constraint, i.e. $\sum_{j}^\text{NS} \mathcal{N}_j U_j^\circ$}
\nomenclature{$h'$}{Sum of contributions from each species towards enthalpy  constraint, i.e. $\sum_{j}^\text{NS} \mathcal{N}_j H_j^\circ$}
...
\subsection{Base equations and derivatives:}
...
\subsection{Notes on derivatives and Bridgmann tables}
...
\subsection{Temperature derivatives and $c_p$}
...
\vspace{5mm}
\\The `--species` option tells the program which species you want to include in the minimization process. You can list your own with comma separated values as such: `--species= N2, O2, NO, N, O`. If you choose to build a species list this way, you will need to specify a couple more things like `--elements` and `--Y`. If instead you use an already created mixture (available ones are air5, air7, air11, air13, mars8), you can do so with `--species= air11`. This will fill in the `--elements` and `--Y` flags for you as well. You can always double check what needs to be filled in with the `show` command.
\vspace{5mm}
...
\begin{itemize}
    ...
    \item \verb|GasType::MARS8| - contains $CO_2$, $N_2$, $O_2$, $CO$, $O$, $C$, $NO$, $N$.
\end{itemize}
...
\vspace{5mm}
\\ This function type takes in two variables that correspond to its constraint type. It then initializes the solution variables with the initial condition $\mathcal{N} = 0.1$, $\mathcal{N}_j = \mathcal{N} / \text{NS}$, or takes the previous solution's $\mathcal{N}_j$'s if possible. Temperature is either initialized as $T = 3800$ K, or the last solution's temperature. 
\vspace{5mm}
\\ If temperature is solved for, the \verb|NASA_fits()| function is called for within the convergence loop, otherwise it is called before it. Inside the convergence loop, namespace functions depending on the energy type are used to fill the Jacobian and RHS vector. An example combination is:
...
Once the system is formed, a LU matrix solver is called to compute $Ax = B$. With the solution, we recover our $\Delta \ln \mathcal{N}_j$. The damping coefficient, $\epsilon$, is then computed based on this update and then we solve for $\mathcal{N}_j$. The convergence is then checked and if we are within our tolerance, we exit the loop and compute mixture properties and derivatives. 

\subsubsection{NASA\_fits()}
This function computes the NASA polynomials that are used in the minimization function. It first finds the temperature range to select the correct coefficients, then it creates an array of temperature values to be used in the calculatation. It computes $H^\circ/\hat{R}T$, $U^\circ/\hat{R}T$, $S^\circ/\hat{R}$, $c_p^\circ/\hat{R}$, and $\mu^\circ/\hat{R}T$ using:

\begin{multline}
    \frac{c_p^\circ(T)}{\hat{R}} = a_1 T^{-2} + a_2 T^{-1} + a_3 + a_4 T + a_5 T^2 + a_6 T^3 + a_7 T^4
\end{multline}
\begin{multline}
    \\ \frac{H(T)^\circ}{\hat{R}T} = -a_1 T^{-2} + a_2 \ln T / T + a_3 + a_4 T / 2 + a_5 T^2 / 3 
    \\ + a_6 T^3 / 4 + a_7 T^4 / 5 + b_1 / T
\end{multline}
\begin{multline}
    \\ \frac{S(T)^\circ}{\hat{R}} = -a_1 T^{-2} / 2 - a_2 T ^ {-1} + a_3 \ln T + a_4 T + a_5 T^2 / 2 
    \\ + a_6 T^3 / 3 + a_7 T^4 / 4 + b_2
\end{multline}
\begin{equation}
    \frac{\mu^\circ}{\hat{R}T} = \frac{H^\circ}{\hat{R}T} - \frac{S^\circ}{\hat{R}}
\end{equation}
\begin{equation}
    \frac{U(T)^\circ}{\hat{R}T} = \frac{H(T)^\circ}{\hat{R}T} - 1
\end{equation}
\begin{equation}
    \frac{c_v^\circ(T)}{\hat{R}} = \frac{c_p^\circ(T)}{\hat{R}} - 1
\end{equation}
\subsubsection{Namespace functions}
The \verb|helm::| and \verb|gibbs::| namespace functions are designed to fill their respective parts of the Jacobian and RHS vector (F) in a very modular way. Very few if statements are used. They all have roughly the same form, and fill the reduced Gibbs/Helmholtz equations sums in an efficient manner. Two commonly named functions are \verb|form_elemental(J, F, gas)| and \verb|form_charge(J, F, gas)|. These functions go through and sum up terms that are needed and place them in their respective entries. Since the elemental constraint it called in every constraint type, there are no \verb|if| statements inside of it. Since many times the charge constraint is used without a temperature constraint, there is no if statement inside \verb|form_charge()| either. Inside the temperature row functions, such as \verb|helm::form_U|, an \verb|if| statement is used to check is the charge constraint is being enforced. By the coloring in the reduced equations, you can see why the \verb|if| function is needed. The temperature rows have a charge term in it, so it checks is \verb|gas.HAS_IONS| is true to see if it needs to add an entry into that row / column.
\newpage
An example of this shown in \verb|helm::form_U()|:
...
\subsubsection{compute\_mixture\_properties()}
    ...
    \begin{align}
        X_j &= \mathcal{N}_j / \mathcal{N}
        \\ \mathcal{M} &= \sum_j^\text{NS} X_j \mathcal{M}_j
        \\ R &= \mathcal{N} \hat{R}
        \\ Y_j &= (X_j \mathcal{M}_j) / \mathcal{M}
    \end{align}

\subsubsection{compute\_derivatives() / compute\_derivativesCFD()}
This function computes the thermodynamic derivatives using the methods outlined in the \textit{CFD derivatives} section. There are two functions since the Gibbs/Helmholtz Jacobians have either $H^\circ$ or $U^\circ$ terms, so some math needs to be changed for each function. A neat math trick is used to minimize the comnputation through the use of the linearity of sums, i.e.:

\begin{equation}
    \sum_{j=1}^\text{NS} a_{ij} \mathcal{N}_j \frac{U_j^\circ}{\hat{R}T} = \sum_{j=1}^\text{NS} a_{ij} \mathcal{N}_j \left(\frac{H_j^\circ}{\hat{R}T} - 1 \right) = \sum_{j=1}^\text{NS} a_{ij} \mathcal{N}_j \frac{H_j^\circ}{\hat{R}T} - \sum_{j=1}^\text{NS} a_{ij} \mathcal{N}_j
\end{equation}

Where both terms are usually computed already at some point.
...
\subsubsection{check\_convergence()}
    ...
    \begin{enumerate}
        \item $\mathcal{N}_j |\Delta \ln \mathcal{N}_j| / \sum_j \mathcal{N}_j \leq 0.5 \cdot 10^{-5}$ 
        \item $\mathcal{N} |\Delta \ln \mathcal{N}| / \sum_j \mathcal{N}_j \leq 0.5 \cdot 10^{-5}$
        \item $|\Delta \ln T| \leq 10^{-4}$
    \end{enumerate}

\subsubsection{compute\_damping()}

The damping parameter $\epsilon$ is calculated in this fucntion as followed. First, $\lambda_1$ is defined as:

\begin{equation}
    \lambda_1 = \frac{2}{\text{max}(5|\Delta \ln T|, 5|\Delta \ln \mathcal{N}|, |\Delta \ln \mathcal{N}_j)}
\end{equation}

If $\ln (\mathcal{N}_j / \mathcal{N}) \leq -\ln(10^{-8})$ and $\Delta \ln \mathcal{N}_j \geq 0$, then we create $\lambda_2$ for that species:

\begin{equation}
    \lambda_2 = \text{min} \left| \frac{-\ln \frac{\mathcal{N}_j}{\mathcal{N}} - 9.2103404}{\Delta \ln \mathcal{N}_j - \Delta \ln \mathcal{N}} \right|
\end{equation}

$\epsilon$ is then found as min(1, $\lambda_1$, $\lambda_2$). $\mathcal{N}_j$ is then calculated as:

\begin{equation}
    \mathcal{N}_j^{k + 1} = \mathcal{N}_j^k \exp (\epsilon \Delta \ln \mathcal{N}_j)
\end{equation}
